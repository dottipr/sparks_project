...
for sample_id in sample_ids:
    ...
    logger.info(f"\tTesting dataset of movie {testing_dataset.video_name} "\
                f"\tcontains {len(testing_dataset)} samples.")

    logger.info(f"\tProcessing samples in UNet...")
    # ys and preds are dictionaries
    _, ys, preds = run_samples_in_model(network=network,
                                        device=device,
                                        datasets=[testing_dataset],
    ...
    write_videos_on_disk(training_name=output_name,
                         video_name=video_name,
                         path=save_folder,
                         preds=preds[testing_dataset.video_name],
                         ys=ys[testing_dataset.video_name]
                        )
