...
class SparkDataset(Dataset):
    ...
    def __init__(self, base_path, sample_ids, testing, params,
                 resampling=False, resampling_rate=150,  # to be implemented
        ...
        self.data = [torch.from_numpy(movie.astype('int'))
                     for movie in self.get_data()]  # int32
    ...
    def preprocess_videos(self):
        ...
        if self.params.data_smoothing == '2d':
            ...
            self.data = [convolve(video, smooth_filter)
    ...
    def adjust_videos_shape(self):
        ...
        self.data = [self.pad_extremities_of_video(
            video) for video in self.data]
        if self.gt_available:
            self.annotations = [self.pad_extremities_of_video(mask,
                                mask=True, padding_value=config.ignore_index)
                                for mask in self.annotations]
    ...
    def pad_extremities_of_video(self, video, mask=False, padding_value=0):
        ...
        assert ((length-self.duration) /
                self.params.data_step) % 1 == 0, "padding at end of video is wrong"
    ...
    def get_ground_truth(self):
        ...
        annotations = [torch.from_numpy(mask.astype('int8'))
                       for mask in annotations]

        if self.testing:  # load event instances
            assert len(
                self.sample_ids) == 1, "Testing mode supports a single video."
    ...
    def __getitem__(self, idx):
        ...
        chunks = self.get_chunks(video_length,
                                 self.params.data_step,
                                 self.params.data_duration)
    ...
    def shrink_mask(self, mask):
        ...
        """
        assert mask.shape[0] % self.params.num_channels == 0, \
...
class SparkDatasetPath(SparkDataset):
    ...
    def __init__(
            self, sample_path, params, resampling=False, resampling_rate=150):
        ...
        super().__init__(
            ...
            testing=False, params=params, resampling=resampling,
            resampling_rate=resampling_rate, gt_available=False,
...
class SparkDatasetLSTM(SparkDataset):
    ...
    def __init__(self, base_path, sample_ids, testing, params,
        ...
        super().__init__(base_path=base_path, sample_ids=sample_ids,
                         testing=testing, params=params, resampling=resampling,
                         resampling_rate=resampling_rate, gt_available=gt_available, inference=inference)
    ...
    def pad_extremities_of_video(self, video, mask=False, padding_value=0):
        ...
        assert video.shape[0] == length + self.params.data_duration - 1, \
