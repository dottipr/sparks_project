...
def paste_segmentation_on_video(video, colored_mask):
    ...
    for frame, ann in zip(video, colored_mask):
        ...
        if len(ann.split()) < 4:
            raise ValueError(
                "Colored mask must have an alpha channel (RGBA format).")
...
def l2_distance(point1, point2):
    """
    This function calculates the L2 Euclidean distance between two 3D points 
    specified by their coordinates.

    Args:
        point1 (tuple): Coordinates of the first point (t1, y1, x1).
        point2 (tuple): Coordinates of the second point (t2, y2, x2).
    ...
    t1, y1, x1 = point1
    t2, y2, x2 = point2
    return math.sqrt((t1 - t2)**2 + (y1 - y2)**2 + (x1 - x2)**2)
...
def ball(center, radius):
    ...
    ball_indices = [
        pt for pt in cube_indices if l2_distance(center, pt) <= radius]
...
def add_colored_paired_sparks_to_video(movie, paired_true, paired_preds, fp, fn,
    ...
    annotated_video = add_colored_locations_to_video(
        paired_true, rgb_video, [0, 255, 0], 0.8 * transparency)  # green
    annotated_video = add_colored_locations_to_video(
        paired_preds, annotated_video, [0, 255, 200], 0.8 * transparency)  # cyan
    annotated_video = add_colored_locations_to_video(
        fp, annotated_video, [255, 255, 0], transparency)  # yellow
    annotated_video = add_colored_locations_to_video(
        fn, annotated_video, [255, 0, 0], transparency)  # red
...
def get_spark_signal(
    ...
    signal = np.average(
        video[start:stop], axis=(1, 2), weights=signal_mask[start:stop])
