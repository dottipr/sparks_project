...
from scipy import spatial, signal, fftpack
...
def keep_percentile(movie, roi_mask, k = 75):
    ...
    roi_sizes = (np.max(np.where(new_roi_mask), axis=1)
                  -np.min(np.where(new_roi_mask), axis=1))
    if roi_sizes.min() < 2:
        new_roi_mask = ndi.binary_dilation(new_roi_mask,
                                           iterations=1)
    new_roi_mask = get_convex_hull(new_roi_mask)[0]
    return new_roi_mask


def reduce_sparks_size(movie, class_mask, event_mask,
                       sigma=2, k = 75):
    '''
    ...
    for id_roi in event_list:
        ...
        new_roi_mask = keep_percentile(movie=movie,
                                       roi_mask=roi_mask,
                                       k=k)
...
def get_new_mask(video, mask, min_dist_xy = MIN_DIST_XY, min_dist_t = MIN_DIST_T,
    ...
    if 1 in mask:
        ...
        sparks_loc, sparks_mask = simple_nonmaxima_suppression(img=video,
...
def sparks_connectivity_mask(min_dist_xy=MIN_DIST_XY,
...
def get_smallest_event(events_mask, event_type):
    ...
    '''
    assert event_type == 'sparks' or event_type == 'puffs', \
    ...
    if events_mask.any():
        ...
        events_durations = [event_slice[0].stop - event_slice[0].start
                           for event_slice in event_slices
                           if event_slice is not None]
        shortest_event = np.min(events_durations)

        if event_type == 'sparks':
            events_widths = [min(event_slice[1].stop - event_slice[1].start,
                                 event_slice[2].stop - event_slice[2].start)
                               for event_slice in event_slices]
            smallest_event = np.min(events_widths)

    if event_type == 'sparks': return shortest_event, smallest_event
    elif event_type == 'puffs': return shortest_event
...
def renumber_labelled_mask(labelled_mask, shift_id = 0):
    ...
    if labelled_mask.max() > 0:
        ...
        assert len(old_labels) == np.max(new_labels)-shift_id, \
...
def get_event_instances_class(event_instances, class_labels,
                              shift_ids = False, to_tensor = False):
    r'''
...
def remove_small_events(class_instances, class_type,
                        min_t=None, min_width=None, max_gap=None):
    ...
    if class_type in ['puffs', 'waves']:
        ...
        class_instances = merge_labels(labelled_mask=class_instances,
                                       max_gap=max_gap)
    ...
    for idx in events_ids:
        ...
        if class_type in ['sparks', 'waves']:
            assert min_width is not None, \
                   "Provide 'min_width' with sparks and waves."

            # if event is too small, remove it from predictions
            if class_type == 'sparks':
                event_width = min(slices[1].stop - slices[1].start,
...
def get_processed_result(sparks, puffs, waves, xs, conn_mask, connectivity,
                         max_gap, sigma, wave_min_width, puff_min_t,
                         spark_min_t, spark_min_width):
    '''
    From raw sparks, puffs and waves preds, get segmented output and event
    instances.
    ...
    preds_segmentation, _ = get_argmax_segmentation_otsu(preds=preds,
                                                         get_classes=True,
                                                         debug=True)
    ...
    for event_type in ['sparks', 'puffs', 'waves']:
        preds_instances[event_type] = renumber_labelled_mask(
                                        labelled_mask=preds_instances[event_type],
                                        shift_id=shift_id)
...
def detect_spark_peaks(movie, spark_mask,
...
def get_sparks_locations_from_mask(mask,
    ...
    #sparks_mask = empty_marginal_frames(sparks_mask, ignore_frames)
    coords = simple_nonmaxima_suppression(img=sparks_mask,
                                          min_dist=connectivity_mask,
                                          threshold=0,
                                          sigma=0.5)
...
def process_spark_prediction(pred,
    ...
    argwhere, argmaxima = simple_nonmaxima_suppression(img=movie,
