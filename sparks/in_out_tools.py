...
from visualization_tools import (add_colored_locations_to_video,
...
def load_predictions_all_trainings_ids(training_names, epochs, metrics_folder, ids):
    ...
    for name, epoch in zip(training_names, epochs):
        ys[name],s[name],p[name],w[name] = load_predictions_ids(training_name,
                                                                epoch,
                                                                metrics_folder,
                                                                ids)
...
def write_colored_class_videos_on_disk(movie, classes_mask, out_dir,
                                       movie_fn, transparency=50,
    ...
    colored_movie = add_colored_classes_to_video(movie, classes_mask,
                                                 transparency, ignore_frames,
                                                 white_bg)

    imageio.volwrite(os.path.join(out_dir,movie_fn+".tif"),
                     colored_movie)
...
def write_spark_locations_on_disk(spark_pred, movie, filename,
                                  t_detection_sparks, min_r_sparks,
                                  ignore_frames=0):
        # process predictions
        sparks_list = process_spark_prediction(pred=spark_pred,
                                               movie=movie,
...
def write_colored_sparks_on_disk(training_name, video_name,
    ...
    if not isinstance(xs, type(None)):
        ...
        assert not isinstance(movie_shape, type(None)), \
               "Provide movie shape if not providing input movie."
        sample_video = 255*np.ones(movie_shape)

    # compute colored sparks mask
    transparency = 45
    annotated_video = add_colored_paired_sparks_to_video(movie=sample_video,
    ...
    # save video on disk
    imageio.volwrite(os.path.join(path, f"{out_name_root}_colored_sparks.tif"),
                     annotated_video)

    # write sparks locations to file
    file_path = os.path.join(path, f"{out_name_root}_sparks_location.txt")
    write_paired_sparks_on_disk(paired_true=paired_real,
                                paired_preds=paired_pred,
                                fp=false_positives,
                                fn=false_negatives,
                                file_path=file_path)
...
def pair_and_write_sparks_on_video(movie, coords_true, coords_preds,
                                   min_dist_xy, min_dist_t, out_path,
                                   white_background, transparency=50,
                                   training_name=None, epoch=None,
                                   movie_id=None):
    ...
    paired_true, paired_preds, fp, fn = correspondences_precision_recall(coords_true,
                                                                         coords_preds,
                                                                         min_dist_t,
                                                                         min_dist_xy,
    ...
    write_paired_sparks_on_disk(paired_true=paired_true,
                                paired_preds=paired_preds,
                                fp=fp,
                                fn=fn,
    ...
    summary_file_path = os.path.join(out_path,
                                     f"parameters{wb_fn}.txt")
