...
def main():
    parser = argparse.ArgumentParser(
        description="Generate u-net segmentations")
    parser.add_argument(
        "sparks",
    ...
    out_folder = os.path.join(
        raw_data_directory, f"unet_masks_{args.sparks}_sparks")
    ...
    for id in args.sample_ids:
        ...
        old_mask = load_annotations_ids(
            data_folder=old_mask_folder, ids=[id], mask_names='class_label')[id]
        ...
        if args.sparks == 'peaks':
            ...
            out_path = os.path.join(out_folder,
                                    f"{id}_class_label_peaks.tif")

        elif args.sparks == 'dilated':
            mask = get_mask_dilated_events(mask=old_mask,
                                           list_radius_ignore=radius_ignore_list,
                                           list_erosion=erosion_list
                                           )

            out_path = os.path.join(out_folder,
                                    f"{id}_class_label_dilated.tif")
            ...
            out_path = os.path.join(out_folder,
                                    f"{id}_class_label_small_peaks.tif")
