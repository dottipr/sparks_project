...
dataset = MaskTEMPDataset(base_path=dataset_path, smoothing='2d',
...
test_files_names = sorted([".".join(f.split(".")[:-1]) for f in
                   os.listdir(os.path.join(dataset_path,"video_temp_test"))])
testing_datasets = [MaskTEMPTestDataset(base_path=dataset_path,
