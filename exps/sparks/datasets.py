...
__all__ = ["SparkDataset", "SparkTestDataset",
           "SparkWaveDataset", "SparkWaveTestDataset",
           "SparkPuffDataset", "SparkPuffTestDataset",
           "ChannelsDataset", "ChannelsTestDataset",
           "MaskDataset", "MaskTestDataset",
           "MaskTEMPDataset", "MaskTEMPTestDataset"]
...
class SparkDataset(Dataset):
    ...
    def __init__(self,
                 base_path=os.path.join(basepath,"..","..","data"),
                 step = 4, duration = 16,
                 smoothing = False):
        self.base_path = base_path
        self.files = sorted(glob.glob(os.path.join(self.base_path, "video", "*.tif")))
        ...
        if smoothing == '2d':
            ...
            self.data = [np.asarray([convolve2d(frame, _smooth_filter, mode = 'same', boundary = 'symm') for frame in sample]) for sample in self.data]

        if smoothing == '3d':
            _smooth_filter = 1/52*np.ones((3,3,3))
            _smooth_filter[1,1,1] = 1/2
            self.data = [convolve(sample, _smooth_filter) for sample in self.data]

        self.step = step
        self.duration = duration

        self.lengths = [video.shape[0] for video in self.data]
        self.blocks_number = [((length-self.duration)//self.step)+1 for length in self.lengths] # blocks in each video
        self.preceding_blocks = np.roll(np.cumsum(self.blocks_number),1)
        self.tot_blocks = self.preceding_blocks[0] # number of blocks in preceding videos in data
        self.preceding_blocks[0] = 0

        self.csv_files = sorted(glob.glob(os.path.join(self.base_path, "sparks", "*.csv")))
        self.csv_data = [pd.read_csv(file).values.astype('int') for file in self.csv_files]
        self.masks = [numpy_to_mask(csv, video.shape) for csv,video in zip(self.csv_data,self.data)]
        self.masks = [final_mask(mask, 2, 3) for mask in self.masks]
    ...
    def __getitem__(self, idx):
        ...
        labels = self.masks[vid_id][chunks[chunk_id]]
...
class SparkTestDataset(Dataset): # dataset that load a single video for testing
    ...
    def __init__(self,
        ...
        if (((self.length-self.duration)/self.step) % 1 != 0):
            self.pad = self.duration + self.step*(1+(self.length-self.duration)//self.step) - self.length
            self.video = np.pad(img,((0,self.pad),(0,0),(0,0)),'constant',constant_values=0)
        ...
        self.mask = numpy_to_mask(self.csv_data, self.video.shape)
        self.mask = final_mask(self.mask, 2, 3)
...
# same datasets but with annotated waves (and other similar events)


class SparkWaveDataset(Dataset):
    ...
    def __init__(self,
        ...
        if resampling:
            ...
            self.data = [video_spline_interpolation(video, video_path, resampling_rate) for video,video_path in zip(self.data,self.files)]
        ...
        self.csv_files = sorted(glob.glob(os.path.join(self.base_path, "sparks", "*.csv")))
        self.csv_data = [pd.read_csv(file).values.astype('int') for file in self.csv_files]
        self.sparks = [numpy_to_mask(csv, video.shape) for csv,video in zip(self.csv_data,self.data)]

        # wave annotations
        self.wave_files = sorted(glob.glob(os.path.join(self.base_path, "waves", "*.tif")))
        self.waves = [2*np.asarray(imageio.volread(file)).astype('int') for file in self.wave_files]

        # combine sparks and waves into same array
        self.annotations = [annotations_mask(s,w) for s,w in zip(self.sparks,self.waves)]
...
class SparkWaveTestDataset(Dataset): # dataset that load a single video for testing
    ...
    def __init__(self,
        ...
        # combine sparks and waves into same array
        self.annotations = annotations_mask(self.sparks, self.waves)
...
# same datasets but with annotated puffs and waves (and other similar events)
...
class SparkPuffTestDataset(Dataset): # dataset that load a single video for testing
...
class ChannelsDataset(Dataset):
    ...
    def __init__(self,
        ...
        # puff annotations
        self.puff_files = sorted(glob.glob(os.path.join(self.base_path, "puffs", "*.tif")))
        self.puffs = [3*np.asarray(imageio.volread(file)).astype('int') for file in self.puff_files]

        # combine sparks, waves and puffs into same array
        self.annotations = [annotations_mask_puffs(s,w,p,radius_event) for s,w,p in zip(self.sparks,self.waves,self.puffs)]
    ...
    def __getitem__(self, idx):
        ...
        chunk = chunk / chunk.max()
        chunk = np.float32(chunk)
        chunk = concat_sin_channels(chunk)
...
class ChannelsTestDataset(Dataset): # dataset that load a single video for testing
    ...
    def __init__(self,
        ...
        if (((self.length-self.duration)/self.step) % 1 != 0):
            self.pad = self.duration + self.step*(1+(self.length-self.duration)//self.step) - self.length
            self.video = np.pad(self.video,((0,self.pad),(0,0),(0,0)),'constant',constant_values=0)
            self.length = self.length + self.pad
...
# datasets using new annotation masks
'''
class MaskDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.base_path = base_path
        self.files = sorted(glob.glob(os.path.join(self.base_path, "new_video", "*.tif")))
        ...
        if smoothing == '2d':
            _smooth_filter = torch.tensor(([1/16,1/16,1/16],[1/16,1/2,1/16],[1/16,1/16,1/16]))
            self.data = [np.asarray([convolve2d(frame, _smooth_filter, mode = 'same', boundary = 'symm') for frame in video]) for video in self.data]
        ...
        self.annotations_files = sorted(glob.glob(os.path.join(self.base_path, "new_annotations", "*.tif")))
        self.annotations = [np.asarray(imageio.volread(f)).astype('int') for f in self.annotations_files]
        self.annotations = [self.add_ignore_region(video, self.radius_ignore, self.ignore_index) for video in self.annotations]

    def compute_chunks_indices(self):
        lengths = [video.shape[0] for video in self.data]
        blocks_number = [((length-self.duration)//self.step)+1 for length in lengths] # blocks in each video
        preceding_blocks = np.roll(np.cumsum(blocks_number),1)
        tot_blocks = preceding_blocks[0] # number of blocks in preceding videos in data
    ...
    def add_ignore_region(self, annotation_mask, radius_ignore, ignore_index):
        binary_mask = np.where(np.logical_or(annotation_mask == 0, annotation_mask == ignore_index), 1, 0)
...
class MaskTestDataset(Dataset): # dataset that load a single video for testing
    ...
    def __init__(self,
        ...
        self.base_path = base_path
        self.video_name = video_name
        self.file = os.path.join(self.base_path, "new_video_test",
                                 self.video_name + ".tif")
...
# dataset using new TEMPORARY masks

class MaskTEMPDataset(Dataset):
...
class MaskTEMPTestDataset(Dataset): # dataset that load a single video for testing
    ...
    def __init__(self,
        ...
        self.base_path = base_path
        self.video_name = video_name
        self.file = os.path.join(self.base_path, "video_temp_test",
                                 self.video_name + ".tif")
        ...
        # annotations
        self.annotations_file = os.path.join(self.base_path, "masks_temp_test",
                                             self.video_name + ".tif")
        self.annotations = np.asarray(imageio.volread(self.annotations_file)).astype(int)
...
class MaskTestDataset(Dataset): # dataset that load a single video for testing
    ...
    def __init__(self,
        ...
        self.base_path = base_path
        self.video_name = video_name
        self.file = os.path.join(self.base_path, "centres_videos_test",
                                 self.video_name + ".tif")
